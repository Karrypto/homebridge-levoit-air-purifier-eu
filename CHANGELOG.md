...
### Changed
- Increase the status refresh minimum and default to protect VeSync daily request quotas.
- Reuse short-lived device list responses when multiple state reads happen together.

### Fixed
- Pause VeSync requests temporarily after the daily request quota is reached instead of repeatedly sending rejected requests.
